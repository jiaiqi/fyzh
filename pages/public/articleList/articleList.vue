		...
		<view class="content news-list">
          ...
          'layout-center-multi-image':
            item.cover_pic_style === '下三' && item.icon_image,
				...
				<image class="image-icon" :src="$u.api.getImagePath(item.icon_image)" v-if="item.icon_image">
				</image>
	...
	export default {
		...
		methods: {
			async getListData(type = 'refresh') {
				...
				let url = '/daq/select/srvdaq_cms_content_select'
			...
			toArticle(e) {
				if (e.content_no) {
					let url =
						`/pages/public/article/article?serviceName=srvdaq_cms_content_select&content_no=${e.content_no}`
					uni.navigateTo({
						url: url
					});
				}
