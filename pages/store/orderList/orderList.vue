				...
				<swiper-item class="swiper-item">
					<scroll-view scroll-y @scrolltolower="reachBottom" class="scroll-view">
						<view class="page-box" v-if="loadStatus[0] === 'nomore' && orderList[0].length === 0">
							<view>
								<view class="centre">
									<image class="image" src="https://cdn.uviewui.com/uview/template/taobao-order.png"
										mode=""></image>
									<view class="explain">
										您还没有相关的订单
						...
						<view class="page-box" v-else>
							<view class="order" v-for="(res, index) in orderList[0]" :key="res.id">
								...
								<view class="item" v-for="(item, index) in res.goodsList" :key="index"
									...
									<view class="left">
										<image class="image" :src="item.goodsUrl" mode="aspectFill"
											v-if="item.goodsUrl"></image>
								...
								<view class="bottom">
									...
									<button class="cu-btn line-orange round sm"
										v-if="res.order_state === '待支付' && res.pay_state === '未支付'"
						...
						<view class="page-box" v-else>
							<view class="order" v-for="(res, index) in orderList[1]" :key="res.id">
								...
								<view class="bottom">
									<button class="cu-btn line-red round sm"
										@click.stop="deleteOrder(res)">删除订单</button>
									<button class="cu-btn line-grey round sm"
										@click.stop="cancelOrder(res)">取消订单</button>
					...
					<scroll-view scroll-y class="scroll-view" @scrolltolower="reachBottom">
						...
						<view class="page-box" v-else>
							<view class="order" v-for="(res, index) in orderList[2]" :key="res.id">
								...
								<view class="bottom">
									<view class="more">
										<bx-icon name="more" :size="26" color="rgb(203,203,203)"></bx-icon>
									</view>
					...
					<scroll-view scroll-y class="scroll-view" @scrolltolower="reachBottom">
						...
						<view class="page-box" v-else>
							<view class="order" v-for="(res, index) in orderList[4]" :key="res.id">
								...
								<view class="bottom">
									<view class="more">
										<bx-icon name="more" :size="26" color="rgb(94,94,94)"></bx-icon>
									</view>
									<button class="cu-btn line-cyan round sm">再次购买</button>
								</view>
							</view>
		...
		methods: {
			...
			deleteOrder(e) {
				...
				uni.showModal({
					...
					success(res) {
						if (res.confirm) {
							let req = [{
								serviceName: 'srvhealth_store_order_delete',
								condition: [{
									colName: 'id',
									ruleType: 'in',
									value: e.id
			...
			reachBottom() {
				...
				if (this.loadStatus[this.current] == 'loadmore') {
					...
					setTimeout(() => {
						this.getOrderList(this.current, 'loadmore');
					}, 1200);
					// }
				}
			},
			...
			async getOrderList(idx) {
				...
				let req = {
					...
					page: {
						pageNo: this.orderList[idx].length === 0 ? this.listPage[idx].pageNo : this.listPage[idx]
							.pageNo + 1,
				...
				if (orderList.state == 'SUCCESS') {
						...
						let data = {
							...
							goodsList: goodsList.reduce((pre, goods) => {
								if (goods.order_no === item.order_no) {
									pre.push({
										goodsUrl: this.$u.getImagePath(goods.goods_image),
										title: goods.goods_name,
										// type: '4K，广色域',
										// deliveryTime: '珍藏10年好酒',
										price: goods.unit_price || 0,
										number: goods.amount
									});
								}
								return pre;
					...
					let originData = this.$u.deepClone(this.orderList[idx]);
					...
					if (orderList.page && orderList.page.pageNo) {
						if (orderList.page.total <= orderList.page.rownumber * orderList.page.pageNo) {
							...
							this.$set(this.loadStatus, idx, 'loadmore');
						}
					} else {
						this.$set(this.loadStatus, idx, 'loadmore');
					}
			...
			change(index) {
				...
				if (this.orderList[index].length === 0 && this.loadStatus[index] === 'loadmore') {
					//
